...
public class RrdCommon {
    ...
    static {
        ...
        rrd = Rrd.getInstance();
        wholeDir = "%OS_RRDPATH%/ipm/rrd/interfaces/device.2/hosts/%IP0%/%IP1%/%IP2%/%IP3%/%PORT%/";
        wholeName = "%OS_RRDPATH%/ipm/rrd/interfaces/device.2/hosts/%IP0%/%IP1%/%IP2%/%IP3%/%PORT%/%KPINAME%.rrd";
        createCmd = "create %OS_RRDPATH%/ipm/rrd/interfaces/device.2/hosts/" + 
            "%IP0%/%IP1%/%IP2%/%IP3%/%PORT%/%KPINAME%.rrd " +
            "--start %TIME% --step 10 DS:VALUE:GAUGE:30:U:U " + 
            "RRA:AVERAGE:0.5:1:144000";
        updateCmd = "update %OS_RRDPATH%/ipm/rrd/interfaces/device.2/hosts/" + 
            "%IP0%/%IP1%/%IP2%/%IP3%/%PORT%/%KPINAME%.rrd " +
            "%TIME%:%VALUE%";
        fetchHostCmd = "fetch %OS_RRDPATH%/ipm/rrd/interfaces/device.2/hosts/" + 
            "%IP0%/%IP1%/%IP2%/%IP3%/%PORT%/%KPINAME%.rrd " + 
            "AVERAGE --start %STARTTIME% --end %ENDTIME%";// --resolution 10"; 
        ...
        if (osName.equals("Linux")) {
            ...
            isOsLinux = false;
            wholeDir = wholeDir.replaceFirst("%OS_RRDPATH%",   "C:/kpi/");
            wholeName = wholeName.replaceFirst("%OS_RRDPATH%", "C:/kpi/");
            createCmd = createCmd.replaceFirst("%OS_RRDPATH%", "C:/kpi/");
            updateCmd = updateCmd.replaceFirst("%OS_RRDPATH%", "C:/kpi/");
            fetchHostCmd = fetchHostCmd.replaceFirst("%OS_RRDPATH%", "C:/kpi/");
        }
    }
     ...
     */
    public static void rrdWriteHostData(long time, ManagerBean mb, String kpiName, double value) {
        try {
            String update = updateCmd;
            String[] ips = mb.getIp().split("\\.");
            time = Util.normalize(time, 10);
            update = update
            ...
            RrdCommon.createDir(time, mb, kpiName);
    ...
    private static void createDir(long time, ManagerBean mb, String kpiName) throws Exception {
    ...
    public static void main(String[] args) {
        ...
        mb.setPort("21");
        mb.setIp("61.138.125.44");
        double a = 1.5;

        while (true) {
            try {
                long now = System.currentTimeMillis() / 1000;
                rrdWriteHostData(now, mb, "test", a);
                System.out.println(rrdFetchHost(now - 60, now, mb, "test"));
                Thread.sleep(11000);
                a += 1;
            } catch (Exception e) {
                e.printStackTrace(); 
